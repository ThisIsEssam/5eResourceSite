...
import { warforgedData } from '../data/warforgedData';
...
function generateFirstName(gender) {
  const col1 = randomItem(warforgedData.name1_col1);

  if (gender === "male") {
    if (maybe(50)) {
      // name1_col1 + name1_col2
      return col1 + randomItem(warforgedData.name1_col2);
    } else {
      // name1_col1 + male suffix
      return col1 + randomItem(warforgedData.name1_male_suffixes);
    }
  }
  ...
  if (gender === "female") {
    if (maybe(50)) {
      ...
      return col1 + randomItem(warforgedData.name1_female_suffixes);
    } else {
      // name1_col1 + name1_col2 + female suffix
      const col2 = randomItem(warforgedData.name1_col2);
      const suffix = randomItem(warforgedData.name1_female_suffixes);
...
function generateLastName() {
  const col1 = randomItem(warforgedData.name2_col1);
